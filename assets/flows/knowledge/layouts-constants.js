import languages_modal from './layouts/languages-modal.json';
import countries_codes_modal from './layouts/countries-codes-modal.json';
import camera_roll_picker_modal from './layouts/camera-roll-picker-modal.json';
import role_modal from './layouts/role-modal.json';
import wallet_modal from './layouts/wallet-modal.json';
import warehouse_modal from './layouts/warehouse-modal.json';
import product_categories_modal from './layouts/product-categories-modal.json';
import product_features_modal from './layouts/product-features-modal.json';
import product_unit_depended_modal from './layouts/product-unit-depended-modal.json';
import list from './layouts/list.json';

module.exports = {
  languages_modal,
  countries_codes_modal,
  camera_roll_picker_modal,
  role_modal,
  wallet_modal,
  warehouse_modal,
  product_categories_modal,
  product_features_modal,
  product_unit_depended_modal,
  list
};
